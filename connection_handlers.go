...
func ackPacketHandler(c *conn, header parser.Header) error {
	...
	if !ok {
		...
		// TODO this is still not a great solutiuon
		rawFunc = emtpyFH
	}
...
func serverEventPacketHandler(c *conn, event string, header parser.Header) error {
...
func serverConnectPacketHandler(c *conn, header parser.Header) error {
...
func serverDisconnectPacketHandler(c *conn, header parser.Header) error {
